 ...
 */
@FacesConverter("vehiculeConverter")
public class VehiculeConverter implements javax.faces.convert.Converter {
    @Override
    public Object getAsObject(FacesContext facesContext, UIComponent uiComponent, String value) {
        if(value != null && value.trim().length() > 0) {
            try {
                ...
                Vehicule vehicule = employeView.getVehicules().stream().filter(b -> b.getId() == Long.valueOf(value)).findFirst().get();
                return vehicule;
